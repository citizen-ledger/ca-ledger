...
| `pipeline/fetch_compensation_data.py` | Regenerates `compensation-data.js` and `comp/*.json` from the SCO Government Compensation exports. **Cannot fetch its own source**: gcc.sco.ca.gov expressly excludes automated retrieval, so the four CSV files must be downloaded by hand into `pipeline/cache/compensation/`. This and the CSU statements are the site's **two** manual-cache exceptions — named together, because two documented exceptions are a limit and two one-offs are how a rebuild-from-source claim quietly stops being true. Neither can be rebuilt with `--refresh`; a scheduled check opens an issue when either goes stale. |
