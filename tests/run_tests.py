...
def test_derived_artefacts_are_watched():
    """EVERY DERIVED ARTEFACT IS REGENERATED BY ONE COMMAND AND WATCHED HERE.

    main went red between two merges. #117 landed the 34th finding document;
    #118 landed the assertion that every document on disk is in the manifest.
    Each branch was green alone, because neither held the other's half, and
    the manifest was generated before the merge. NOTHING WAS FORGOTTEN — the
    failure needed no mistake, only two correct changes landing in an order
    nobody chose. So a rule that depends on someone remembering to re-run a
    builder cannot be the fix.

    Two things make it structural:

      pipeline/rebuild_derived.py  regenerates every build_*.py, discovered by
                                   glob, and separates substantive change from
                                   the date-stamp churn all three artefacts
                                   produce by construction.
      this test                    asserts each declared artefact against its
                                   OWN SOURCE, so staleness fails whether or
                                   not anyone ran the builder.

    THE META-GUARD IS THE POINT. A new build_*.py must declare OUTPUTS, and
    every name it declares must be checked below. Without that, the next
    derived artefact ships with nothing watching it — which is the defect
    this whole exercise is about, one level up."""
    import re as _re

    builders = sorted((ROOT / "pipeline").glob("build_*.py"))
    check("derived: builders were discovered by glob, not listed",
          len(builders) >= 3, str([b.name for b in builders]))

    declared = {}
    for b in builders:
        src = b.read_text(encoding="utf-8")
        m = _re.search(r"^OUTPUTS = \[([^\]]*)\]", src, _re.M)
        check(f"derived: {b.name} declares what it writes", bool(m),
              "no OUTPUTS list")
        if not m:
            continue
        declared[b.name] = _re.findall(r'"([^"]+)"', m.group(1))

    # every declared artefact is named in this test file, so a new builder
    # cannot ship an artefact nothing watches
    me = (ROOT / "tests" / "run_tests.py").read_text(encoding="utf-8")
    for name, outs in declared.items():
        for o in outs:
            if "*" in o:
                # A SET, not a file. A substring match cannot express "every
                # CSV in bulk/ is covered", and pretending it can would make
                # this guard pass on a literal it found in a comment. What is
                # required instead is that the test ENUMERATE the set from
                # disk — the same derive-don't-list rule the artefacts follow.
                d, pat = o.rsplit("/", 1)
                want = f'(ROOT / "{d}").glob("{pat}")'
                check(f"derived: the set {o} (from {name}) is enumerated from "
                      "disk by an assertion", want in me,
                      f"expected {want} in run_tests.py")
            else:
                check(f"derived: {o} (from {name}) is watched by an assertion",
                      o in me, f"{o} appears nowhere in run_tests.py")

    # ---- FRESHNESS, AGAINST EACH ARTEFACT'S OWN SOURCE.
    # Never a byte comparison against a committed snapshot: all three embed a
    # build date, so bytes differ on any later day while every figure is
    # identical. Measured 2026-07-29 — bulk/compensation.csv differed in three
    # lines (Data generated, Exported, a vintage ageDaysAtBuild of 3 vs 7) and
    # was identical on the other 4,144. A check that red every morning is a
    # check nobody reads.

    # findings-manifest.js against docs/ on disk
    man = load_data_js(ROOT / "findings-manifest.js")
    NOT_FINDINGS = {"OPEN.md", "SCOPE.md", "SECURITY.md", "GUARDRAILS.md"}
    docs = {p.name for p in (ROOT / "docs").glob("*.md")} - NOT_FINDINGS
    listed = {f["doc"].split("/")[-1] for f in man["findings"]}
    check("derived: findings-manifest.js names exactly the documents on disk",
          listed == docs, str(sorted(docs ^ listed)))
    # and each entry's title is still that document's own H1 — a rename would
    # otherwise leave the index describing a document by its former name
    stale = []
    for f in man["findings"]:
        txt = (ROOT / f["doc"]).read_text(encoding="utf-8")
        m = _re.search(r"^#\s+(.+)$", txt, _re.M)
        if m and m.group(1).strip() != f["title"]:
            stale.append(f["doc"])
    check("derived: every manifest title is the document's current H1",
          not stale, str(stale))

    # search-index.js against the payloads it is built from
    ix = load_data_js(ROOT / "search-index.js")
    counts = {}
    for spec, payload, key in (("city", "city-data.js", "cities"),
                               ("county", "county-data.js", "counties"),
                               ("district", "district-data.js", "districts"),
                               ("ccc", "ccc-data.js", "districts"),
                               ("uc", "uc-data.js", "campuses")):
        d = load_data_js(ROOT / payload)
        counts[spec] = len(d[key])
    for spec, n in counts.items():
        got = sum(1 for e in ix["e"] if ix["layers"][e[1]]["key"] == spec)
        check(f"derived: search-index carries every {spec} in its payload "
              f"({n})", got == n, f"index {got} vs payload {n}")
    check("derived: and the index's own declared count matches its entries",
          ix["meta"]["entities"] == len(ix["e"]),
          f'{ix["meta"]["entities"]} vs {len(ix["e"])}')

    # bulk: every shipped layer has an export, and the manifest agrees with
    # the files actually on disk
    bm = load_data_js(ROOT / "bulk-manifest.js")
    on_disk = {p.name for p in (ROOT / "bulk").glob("*.csv")}
    named = {f["file"].split("/")[-1] for f in bm["files"]} if "files" in bm \
        else {k.split("/")[-1] for k in bm}
    check("derived: bulk-manifest.js names exactly the CSVs on disk",
          named == on_disk, str(sorted(named ^ on_disk)))
    schema = (ROOT / "bulk" / "SCHEMA.md").read_text(encoding="utf-8")
    for f in sorted(on_disk):
        check(f"derived: bulk/SCHEMA.md documents {f}", f in schema)

    # ---- rebuild_derived.py exists, globs, and refuses an empty glob
    rd = ROOT / "pipeline" / "rebuild_derived.py"
    check("derived: there is one command that regenerates everything",
          rd.exists())
    rsrc = rd.read_text(encoding="utf-8")
    check("derived: it discovers builders by glob rather than a list",
          'glob("build_*.py")' in rsrc)
    check("derived: and refuses to report success on an empty glob, because "
          "the failure mode of a glob is silence",
          "refusing to report success" in rsrc)
    check("derived: it separates substantive change from date-stamp churn",
          "date-stamp-only churn" in rsrc and "DATE_LINE" in rsrc)
